...
it("returns a 401 if the user is not the owner of the ticket", async () => {
  ...
  await request(app)
    .put(`/api/tickets/${responses.body.id}`)
    .set("Cookie", global.signin())
    .send({
      title: "concertMela",
      price: 200,
...
it("updates a ticket with valid input", async () => {
  const cookie = global.signin();
  const responses = await request(app)
    .post("/api/tickets")
    .set("Cookie", cookie)
  ...
  const ticketResponse = await request(app)
    .get(`/api/tickets/${responses.body.id}`)
    .send();
...
it("rejects updates if the ticket is reserved", async () => {
  ...
  const responses = await request(app)
    ...
    const ticket = await Ticket.findById(responses.body.id);
